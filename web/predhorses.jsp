...
<ul class="menu">
    ...
    <form action="predhorses.jsp" name="form">
...
<div class="advantages">
    <div class="container">
        <div class="advantages-main">
            <h3><fmt:message key="menu_horses"/></h3>
            <c:set var="kek" value="${isCreated}" scope="session"/>
            <c:choose>
                ...
                <c:otherwise>
                    <div class="pred_block">
                        <form name="action" method="post" action="controller">
                            <input type="hidden" name="command" value="predictionaction">
                            <table class="table-fill">
                                <thead>
                                <tr>
                                    <th class="text-left"><fmt:message key="nickname"/></th>
                                    <th class="text-left"><fmt:message key="breed"/></th>
                                    <th class="text-left"><fmt:message key="age"/></th>
                                    <th class="text-left"><fmt:message key="jockey"/></th>
                                    <th class="text-left"><fmt:message key="rating"/></th>
                                </tr>
                                </thead>
                                <jsp:useBean id="horses" scope="session" type="java.util.List"/>
                                <tbody>
                                <c:forEach var="post" items="${horses}">
                                    <tr>
                                        <td class="text-left">${post.nickname}</td>
                                        <td class="text-left">${post.breed}</td>
                                        <td class="text-left">${post.age}</td>
                                        <td class="text-left">${post.jockeyName}</td>
                                        <td class="text-left">${post.rating}</td>
                                    </tr>
                                </c:forEach>
                                </tbody>
                            </table>
