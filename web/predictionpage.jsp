...
<head>
    ...
    <link href="${pageContext.request.contextPath}/predictionfiles/style.css" rel="stylesheet" type="text/css" media="all"/>
...
<script type="text/javascript">
    <c:choose>
    <c:when test="${isFailedShowPredictions == true}">
    if (alert("<fmt:message key="message.checkpredictionshistoryerror"/> ")) {
        ${isFailedShowPredictions = false}
    }
    </c:when>
    <c:when test="${isFailedAddPrediction == true}">
    if (alert("<fmt:message key="message.addpredictionerror"/> ")) {
        ${isFailedAddPrediction = false}
    }
    </c:when>
    <c:when test="${isFailedDeletePrediction == true}">
    if (alert("<fmt:message key="message.deletepredictionerror"/> ")) {
        ${isFailedDeletePrediction = false}
    }
    </c:when>
    <c:when test="${not empty isFailedEditPrediction}">
    if (alert("<fmt:message key="message.editpredictionerror"/> ")) {
        ${isFailedEditPrediction = null}
    }
    </c:when>
    </c:choose>
</script>
<ul class="menu">
    <form action="predictionpage.jsp" name="form">
    ...
    <div class="container">
        <div class="header-main">
            <div class="header-left">
                <div class="logo">
                    <h1><a href="#"><fmt:message key="horseraces"/> </a></h1>
                </div>
                ...
                <form name="action" method="post" action="controller">
                    ...
                    <table class="table-fill">
                        ...
                        <c:forEach var="post" items="${list}">
                            <tr><td class="text-left">${post.id}</td>
                                ...
                                <td class="text-left"><button name="delete" value="${post.id}" type="submit"><fmt:message key="delete"/></button></td><td class="text-left"><button name="edit" value="${post.id}" type="submit"><fmt:message key="edit"/></button></td></tr>
                        </c:forEach>
                        </tbody>
                    </table>
                </form>
