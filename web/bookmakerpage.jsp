...
<head>
   ...
   <link rel="stylesheet" href="./bookmakerfiles/bootstrap.min.css" type="text/css" media="all"/>
    ...
    <script type="text/javascript" src="./bookmakerfiles/jquery-3.2.1.js"></script>
...
<script type="text/javascript">
    <c:choose>
    <c:when test="${isFailedShowHorses == true}">
    if (alert("<fmt:message key="message.showhorseserror"/> ")) {
        ${isFailedShowHorses = false}
    }
    </c:when>
    <c:when test="${isFailedShowRaces == true}">
    if (alert("<fmt:message key="message.showraceserror"/> ")) {
        ${isFailedShowRaces = false}
    }
    </c:when>
    <c:when test="${isFailedAddBet == true}">
    if (alert("<fmt:message key="message.addbeterror"/> ")) {
        ${isFailedAddBet = false}
    }
    </c:when>
    <c:when test="${isFailedDeleteBet == true}">
    if (alert("<fmt:message key="message.deletebeterror"/> ")) {
        ${isFailedDeleteBet = false}
    }
    </c:when>
    <c:when test="${isFailedEditBet == true}">
    if (alert("<fmt:message key="message.editbeterror"/> ")) {
        ${isFailedEditBet = false}
    }
    </c:when>
    <c:when test="${isFailedShowBets == true}">
    if (alert("<fmt:message key="message.checkbetshistoryerror"/> ")) {
        ${isFailedShowBets = false}
    }
    </c:when>
    </c:choose>
</script>
...
<ul class="menu">
    ...
    <form action="bookmakerpage.jsp" name="form">
...
<div class="who">
    <div class="container">
        ...
        <c:when test="${kek == false}">
            <h6><fmt:message key="no_info"/></h6>
        </c:when>
        <c:otherwise>
        <div class = "block">
            <form name="action" method="post" action="controller">
                <input type="hidden" name="command" value="QUOTATION_ACTION">
                <table>
                    <thead>
                    <tr>
                    ...
                    <c:forEach var="post" items="${list}">
                        <tr><td>${post.userId}</td>
                            ...
                            <td><button class="red" name="delete_command" value="${post.id}" type="submit"><fmt:message key="delete"/></button></td><td><button name="edit_command" class="green" value="${post.id}" type="submit"><fmt:message key="edit"/></button></td></tr>
                    </c:forEach>
                    </tbody>
                </table>
            </form>
